...
*** =channel=
The channel to use for =k3s= releases. When not specified =stable= is used, though =latest= often is also safe enough.
...
*** =ip_preference=
For dual stack setups, specify which IP address family to prefer. Defaults to =ipv4=, other possible value is =ipv6=. This will sort multi protocol configuration keys, which also eliminates configuration errors due to wrong order specified there.
...
Can be set to =true= or =false=. This will create test containers to see if the service IPs are reachable from each node. This identifies network issues which would cause interesting behaviour later on, so it is recommended to enable this when creating a new cluster, and disable it after that.

*** =use_config_file=
Specify initial configuration parameters through =/etc/rancher/k3s/config.yaml= instead of using command line arguments. In some cases this can be more reliable.

*** =vacuum_logs=
Vacuum journal before doing a new installation. This is mainly useful when re-deploying on an existing operating system to make sure no errors from the previous installation remain in the logs.
...
*** =k3s_node_labels=
A map of Kubernetes label key -> value to apply to this node. Labels are applied by the first server during each ansible run using a strategic merge patch, so they are idempotent. Labels not mentioned in =k3s_node_labels= are left untouched.

Each entry supports two forms:

- Plain string value -> label set to that value (present)
- Dict with =value= [+ optional =state: present=] -> label set to =value=
- Dict with =state: absent= -> label nulled out (removed from the node)

#+BEGIN_SRC yaml
# host_vars/storage2.yml
k3s_node_labels:
  node-role: storage          # string: set label
  disk-type:
    value: nvme               # dict with value: set label
  retired-label:
    state: absent             # remove label from node
#+END_SRC

Labels applied via =k3s_node_labels= can be referenced in =k3s-pod= definitions using =k3s.node_selector= to constrain which node a pod is scheduled on.
...
- =metallb.yaml= — the rendered metallb pool/advertisement config (present-state entries only)
- =metallb-remove.yaml= — metallb entries marked =state: absent= (removal pass)
- ={pod-name}-{workload-type}.yaml= — the rendered Deployment or DaemonSet for simple-mode pods
- ={pod-name}-manifest.yaml= — the raw manifest for =pod.manifest= pods
...
pod-name:
  ...
  k3s:
    ...
    snippets:                # role names to load pod definitions from
      - myrole               # loads roles/myrole/templates/k3s-pod.yml.j2
    labels:                  # extra labels added to pod template metadata
      my-label: my-value
    node_selector:           # constrain pod scheduling to nodes with these labels
      node-role: storage     # all keys must match; AND semantics
    selector:                # service selector labels (default: {app: pod-name})
      app: pod-name          # override when using pod.manifest with non-default pod labels
      component: backend
  network:
    if1:                     # interface key; becomes part of Service name: <pod>-if1
      ipv4: 1.2.3.4/24       # IPv4 metallb LoadBalancer IP (CIDR notation)
      ipv6: "fd00::1/48"     # IPv6 metallb LoadBalancer IP (CIDR notation)
      ...
      links:                 # alternative to link: aggregate ports from multiple containers
        - container-a
        - container-b
      ports:                 # explicit ports; use with pod.manifest (no containers dict)
        - containerPort: 53
          protocol: UDP
        - containerPort: 53
          protocol: TCP
  pod:
    ...
    configmaps:              # ConfigMaps created and managed alongside the pod
      - name: my-config
        data:
          config.yaml: |
            key: value
      - name: my-authorized-keys  # assembled from authorized_keys.d/ files
        authorized_keys:
          management: true   # include management_keys variable
          key_names:         # additional named keys from authorized_keys.d/
            - deploy-bot
    ...
    volumes:                 # volumes available to containers
      ...
      - name: shared-mem
        emptyDir:
          sizeLimit: 100Mi   # optional size cap; omit for unlimited
      - name: host-logs
        hostPath:
          path: /var/log/pods
    ...
    manifest:                # raw Kubernetes resource dict (mutually exclusive with containers)
      apiVersion: apps/v1
      kind: Deployment
      metadata:
        name: pod-name
        namespace: pod-name
      spec: {}               # full k8s spec; role only wraps namespace/secrets/services
    containers:              # simple mode: role generates a Deployment from this dict
      container-name:
        image: image:tag
        command:             # override image ENTRYPOINT (each element is a separate entry)
          - /usr/sbin/sshd
          - -D
        args:
          - -flag=value      # arguments passed to entrypoint (or command if set)
        ports:
          - containerPort: 80
          - containerPort: 443
            protocol: TCP
        ...
        securityContext:     # optional container security context
          runAsUser: 1000
          runAsGroup: 1000
          runAsNonRoot: true
...
- =node_selector= Map of label key -> value used as a Kubernetes =nodeSelector=. The pod is only scheduled on nodes that carry all specified labels. Omit to allow scheduling on any node.
- =selector= Map of label key -> value used as the =selector= in generated LoadBalancer Services. Defaults to ={app: <pod-name>}=, which matches the label the role adds to pod templates in simple mode. When using =pod.manifest= with custom pod labels, set this to match the =matchLabels= in your manifest so traffic reaches the right pods.
- =snippets= A list of roles to load snippets for pod definitions from. Experimental, see detailed documentation below.
...
When an interface carries only IPv4 addresses, a =SingleStack= IPv4 Service is created. When only IPv6, a =SingleStack= IPv6 Service. When both families are present, a =RequireDualStack= Service is created with the =metallb.universe.tf/loadBalancerIPs= annotation (comma-separated IPv4,IPv6).

To have separate Services for IPv4 and IPv6 — which is the recommended approach when different containers handle the two families — use two interface keys, each with only one address:

#+BEGIN_SRC yaml
network:
  if1:
    ipv4: 192.0.2.10/28
    links: [v4-container, sidecar]
  if2:
    ipv6: "fd42:0:1::1/48"
    links: [v6-container, sidecar]
#+END_SRC

**** Port fields

Exactly one of the following must be set for a Service to be created:

- =ports= Explicit port list on the interface. Use this in raw manifest mode (=pod.manifest=) where there is no containers dict to derive ports from. Format is identical to a Kubernetes container =ports= list.
- =links= List of container names (from =pod.containers=). Port lists from all named containers are merged. Use when one Service IP should expose ports from multiple containers.
- =link= Single container name. Ports are taken from that container's =ports= list.

=ports= takes precedence over =links=/=link=. When the same port number appears with different protocols (e.g. DNS on 53/UDP and 53/TCP), both are included as distinct Service port entries. Port name disambiguation uses the protocol suffix (=port-53-udp=, =port-53-tcp=).

A Service is only created when an address and at least one port source are defined. Stale Services (removed from =network= config) and Services carrying the deprecated =metallb.universe.tf/loadBalancerIPs= annotation on interfaces that are not configured as dual-stack are deleted before current Services are applied.
...
Each entry requires =name= (must be unique within the namespace) and one of the following content sources:
...
- =file= Map of key -> path. Each path is relative to =playbook_dir= and loaded at run time via =lookup('file', ...)=. The file content becomes the ConfigMap value for that key. Fails loudly if a referenced file does not exist.

Use this when the config is maintained as a file in the playbook repository rather than inlined in the host definition.

#+BEGIN_SRC yaml
configmaps:
  - name: app-sidecar-config
    file:
      config.yml: files/app-sidecar-config.yml
#+END_SRC

Like =data= configmaps, changes to file-backed ConfigMaps that are mounted via =subPath= trigger an automatic rolling restart of the workload.

**** Authorized keys

- =authorized_keys= Assembles an =authorized_keys= file from keys stored in =authorized_keys.d/= (the same files used by =basic-host= for management user keys).

  - =management= Boolean. When =true=, includes all keys listed in the =management_keys= playbook variable.
  - =key_names= Optional list of additional key filenames to include from =authorized_keys.d/=.

Both fields are optional and can be combined; management keys come first.

#+BEGIN_SRC yaml
configmaps:
  - name: my-pod-authorized-keys
    authorized_keys:
      management: true       # include all management_keys
      key_names:             # plus any additional named keys
        - deploy-bot
#+END_SRC

Mount it into the container with =subPath: authorized_keys= and point sshd at it via =-o AuthorizedKeysFile=/path/authorized_keys=. When sshd runs as a non-root user, also pass =-o StrictModes=no= since ConfigMap-mounted files are owned by root.
...
Ephemeral volume, lost when the pod restarts. Useful for shared scratch space between containers or for state that does not need to persist. An optional =sizeLimit= caps the amount of node storage this volume may consume.
...
A map of container name -> container definitions. Multiple containers in one pod share the same network namespace and can share volumes.

- =image= Container image, including tag. Required.
- =command= List of strings that overrides the container image's =ENTRYPOINT=. Use this to run a different binary than the image default (e.g. starting =sshd= in an image whose default entrypoint is an application process). Each element is a separate list entry; flags and their values must be separate entries. When both =command= and =args= are set, =command= is the executable and =args= are its arguments.
- =args= List of command-line arguments passed to the container entrypoint (or to =command= if set).
...
- =securityContext= Container-level security context. Supported fields:
  - =runAsUser= UID to run the container process as.
  - =runAsGroup= GID for the container process.
  - =runAsNonRoot= Require a non-zero UID.
  - =readOnlyRootFilesystem= Mount the container root filesystem read-only. Processes that need to write (logs, temp files) must use =volumeMounts= to writable volumes.
  - =allowPrivilegeEscalation= Allow the container process to gain more privileges than its parent. Set to =false= for least-privilege containers.
  - =privileged= Run the container with full host privileges. Avoid unless absolutely necessary.
  - =capabilities.add= / =capabilities.drop= Linux capability adjustments. Common pattern: =drop: [ALL]= then =add:= only what is needed.
...
A list of init containers that run to completion before any main container starts, see the [[https://kubernetes.io/docs/concepts/workloads/pods/init-containers/][k8s documentation]] for more details.

Each entry supports:
- =name= Init container name. Required.
- =image= Container image. Required.
- =command= List of strings forming the command to run (overrides the image entrypoint).
- =volumeMounts= List of volume mounts (same format as in =pod.containers=).

Init containers always run as root unless the image specifies otherwise; they do not inherit the =securityContext= of the main containers.

A common use case is fixing permissions on an =emptyDir= volume before read-only main containers chroot into it:
...
A raw Kubernetes manifest, useful for more complex setups, when configuration should stay as close to vanilla k8s as possible, or when the automations this role is offering don't offer more value than sticking to k8s notation. The role applies it with =apply= semantics and continues to handle everything around it: namespace creation, secret generation, and LoadBalancer Services derived from =network=.

=pod.manifest= and =pod.containers= are mutually exclusive. If both are present, =pod.manifest= wins and =pod.containers= is ignored.

Because there is no containers dict to derive ports from they must be explicitly declared on each network interface using the =ports= key. The manifest namespace must match =k3s.namespace= (defaulting to the pod name): the role creates the namespace before applying the manifest.

If the manifest's pod template uses labels other than =app: <pod-name>=, set =k3s.selector= to match those labels so the generated Services route correctly.

#+BEGIN_SRC yaml
mypod:
  type: k3s-pod
  groups: k3s_mycluster
  k3s:
    cluster: storage1
    selector:
      app: mypod
        ...
        template:
          ...
          spec:
            initContainers:
              - name: init-config
                image: busybox
                command: [sh, -c, "cp /etc/secrets/* /run/config/"]
            containers:
              - name: web
...
The =k3s.pod_snippets= key in a site definition takes a list of roles. For each named role, the inventory plugin included with this role loads =templates/k3s-pod.yml.j2= from that role's directory and merges its =pod:= section into the host definition. This happens at inventory construction time, so the merged container/volume layout is available for DNS generation, group membership, and the sshkubectl per-container host entries.

Snippets are merged in list order (later entries override earlier ones). The host's own =pod:= block is always applied last and wins over all snippets. The behaviour in more detail is:

- =containers= — deep merge per container; the overriding side wins per field. Nested dicts (=env=, =resources=) are themselves merged so adding one env var does not wipe the rest.
- =volumes=, =configmaps=, =secrets= — list merge by =name=; an overriding entry with the same name replaces the base entry; unique entries from both sides are kept.
- =tolerations= — concatenated (base first, then override).
...
my-postgres:
  ...
  k3s:
    cluster: storage1
    snippets:
      - postgresql         # loads roles/postgresql/templates/k3s-pod.yml.j2
  network:
    if1:
      ipv4: 192.168.33.130/24
      link: postgres
  # pod: block is optional; any keys here override the snippet
  pod:
    containers:
      postgres:
        env:
          POSTGRES_DB: mydb    # overrides snippet default
#+END_SRC

#+BEGIN_SRC yaml
# roles/postgresql/templates/k3s-pod.yml.j2
pod:
  containers:
    postgres:
      image: postgres:16
  ...
  volumes:
    - name: pgdata
      persistentVolumeClaim:
        claimName: "{{inventory_hostname}}-pgdata"
        storageClass: longhorn
        size: 20Gi
#+END_SRC

The =roles_path= setting in =ansible.cfg= (under =[site_yaml]=) controls where the plugin searches for roles. When unset, the Ansible default roles path is used.
...
The role can deploy Kubernetes =NetworkPolicy= resources to enforce a default-deny posture with opt-in exceptions. Policies are configured under =network_policies= in the cluster config and applied per namespace.

The default-deny baseline and a DNS egress exception are always created. Additional access is granted by:

- =system_namespaces= — bidirectional access to/from named system namespaces (covers CoreDNS, metrics-server, Prometheus scraping, MetalLB health checks, etc.) without requiring pod labels.
- =dmz_cidrs= — ingress from and egress to external CIDRs for pods labelled =network/dmz: "true"=; cluster-internal node CIDRs (=k3s_cidrs=) are excluded from egress by address family.
- =service_vlans= — per-VLAN ingress/egress policies; VLANs can reference =dhcp_networks= inventory entries or use explicit CIDRs. Pods use =network/<vlan>: "true"= to receive traffic from the VLAN, and =network/<vlan>-access: "true"= to initiate traffic to it.
- Label =network/cluster-service: "true"= makes a pod reachable from any pod in any managed namespace without CIDR rules.

Pod labels go in =k3s.labels= on a =k3s-pod= definition and are applied to the pod template.

Network policy support is very experimental - expect breakage and syntax changes.
...
k3s_clusters:
  my-cluster:
    network_policies:
      ...
      service_vlans:
        db:
          vlan: db             # look up CIDRs from dhcp_networks.db.subnets
        storage:
          cidr: "10.0.30.0/24"
        mgmt:
          cidrs:
